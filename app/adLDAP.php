 ...
 *   http://adldap.sourceforge.net/control.vault.wananchi.com
 *
...
class adLDAP
{
    ...
    /** protected $_domain_controllers = array ("control.vault.wananchi.com"); */

    /* protected $_domain_controllers = array ("192.168.10.90"); */

    protected $_domain_controllers = array("41.212.90.21");
    ...
    public function groups_in_group($group, $recursive = NULL)
    {
        ...
        for ($i = 0; $i < $groups["count"]; $i++) {
            $filter = "(&(objectCategory=group)(distinguishedName=" . $this->ldapSlashes($groups[$i]) . "))";
    ...
    public function group_members($group, $recursive = NULL)
    {
        ...
        for ($i = 0; $i < $users["count"]; $i++) {
            $filter = "(&(objectCategory=person)(distinguishedName=" . $this->ldapSlashes($users[$i]) . "))";
            $fields = array("samaccountname", "distinguishedname", "objectClass");
            $sr = ldap_search($this->_conn, $this->_base_dn, $filter, $fields);
            $entries = ldap_get_entries($this->_conn, $sr);
            ...
            // not a person, look for a group
            if ($entries['count'] == 0 && $recursive == true) {
                $filter = "(&(objectCategory=group)(distinguishedName=" . $this->ldapSlashes($users[$i]) . "))";
    ...
    public function group_info($group_name, $fields = NULL)
    {
        ...
        $filter = "(&(objectCategory=group)(name=" . $this->ldapSlashes($group_name) . "))";
        //echo ($filter."!!!<br>");
        if ($fields === NULL) {
