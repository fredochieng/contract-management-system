        ...
        <div class="row">
            <div class="col-xs-12">
                <h2 class="page-header" style="font-weight:bold">
                    ...
                    <small class="pull-right" style="font-weight:bold">Ticket Number: #
                        {{ $contract->contract_code }}</small>
                </h2>
            </div>
        </div>
        <div class="invoice-info">
            <!-- Contract Details row -->
            {!! Form::open(['action'=>['ContractController@publish',
            $contract->contract_id],'method'=>'POST','class'=>'form','enctype'=>'multipart/form-data'])
            !!}
            <div class="row">
                <div class="col-md-6">
            ...
            <div class="row no-print">
                <div class="col-xs-12">
                    @if (auth()->check()) @if($contract->contract_status=='Created' && (auth()->user()->isLegal() ||
                    auth()->user()->isAdmin()))
                    {!!
                    Form::open(['action'=>['ContractController@publish',$contract->contract_id],'method'=>'POST','class'=>'floatit',
                    'enctype'=>'multipart/form-data'])!!}
                    <a href="#modal_publish_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_publish_contract" class="btn btn-success">
                    ...
                    <a href="#modal_publish_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_publish_contract" class="btn btn-success">
                        <i class="fa fa-check"></i> Publish Contract</a> @endif @endif @if (auth()->check()) @if(
                    $contract->contract_status =='Pending' && auth()->user()->isUser())
                    ...
                    @endif @endif @if (auth()->check()) @if($contract->contract_status=='Pending' &&
                    $contract->assigned== '1' && (auth()->user()->isAdmin()))
                    <a href="#modal_approve_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_approve_contract" class="btn btn-success"><i class="fa fa-check"></i>
                        Approve Contract</a>

                    <a href="#modal_ammend_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_ammend_contract" class="btn btn-info"><i class="fa fa-refresh"></i> Ammend
                        Contract</a>

                    <a href="#modal_terminate_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_terminate_contract" class="btn btn-danger"><i class="fa fa-close"></i>
                        Terminate Contract</a> @elseif($contract->contract_status=='Pending'
                    && $contract->assigned== '1' && $contract->assigned_user_id== Auth::user()->id &&
                    (auth()->user()->isLegal()))
                    <a href="#modal_approve_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_approve_contract" class="btn btn-success"><i class="fa fa-check"></i>
                        Approve Contract</a>
                    ...
                    @elseif($contract->contract_status=='Pending' && $contract->assigned=='' &&
                    (auth()->user()->isLegal())) {!!
                    Form::open(['action'=>['ContractController@workonContract',$contract->contract_id],'method'=>'POST','class'=>'floatit',
                    'enctype'=>'multipart/form-data'])!!}
                    <a href="#modal_work_on" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_work_on" class="btn btn-success">
                    ...
                    'enctype'=>'multipart/form-data'])!!}
                    <a href="#modal_assign_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_assign_contract" class="btn btn-info"><i class="fa fa-check"></i> Assign
                        Contract</a> {!!
                    Form::open(['action'=>['ContractController@workonContract',$contract->contract_id],'method'=>'POST','class'=>'floatit',
                    'enctype'=>'multipart/form-data'])!!}
                    <a href="#modal_work_on" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_work_on" class="btn btn-success"><i class="fa fa-check"></i> Work on
                        Contract</a> @endif @endif @if (auth()->check()) @if($contract->contract_status =='Amended' &&
                    (auth()->user()->isLegal()
                    ...
                    @endif @endif @if (auth()->check()) @if($contract->contract_status =='Amended' &&
                    (auth()->user()->isLegal() || auth()->user()->isAdmin())
                    && $contract->created_by == Auth::user()->id)
                    <a href="/contract/{{$contract->contract_id}}/edit" class="btn btn-success"><i
                            class="fa fa-check"></i> Approve Changes</a> @endif @endif @if (auth()->check())
                    @if($contract->contract_status =='Amended' && (auth()->user()->isUser()))
                    <a href="/contract/{{$contract->contract_id}}/edit" class="btn btn-success"><i
                            class="fa fa-check"></i> Approve Changes</a>
                    {{-- <a href="#modal_approve_changes" data-toggle="modal" data-target="#modal_approve_changes" class="btn btn-success">
                    ...
                    <a href="#modal_reject_changes" data-toggle="modal" data-target="#modal_reject_changes" class="btn btn-danger">
                                                        <i class="fa fa-close"></i> REJECT CHANGES</a>                    --}}
                    @endif @endif @if (auth()->check())
                    @if($contract->contract_status =='Approved' && (auth()->user()->isAdmin()
                    ...
                    @endif @endif @if (auth()->check()) @if($contract->contract_status =='Closed' &&
                    $contract->signed_contract_file == '' &&
                    (auth()->user()->isAdmin() || auth()->user()->isLegal()))
                    <a href="#modal_archive_contract" data-backdrop="static" data-keyboard="false" data-toggle="modal"
                        data-target="#modal_archive_contract" class="btn btn-success"><i
                            class="fa fa-file-archive-o"></i> Archive Contract</a>@endif @endif @if (auth()->check())
                    @if($contract->contract_status =='Closed' && (auth()->user()->isUser()))

                    <p class="col-md-6 text-blue well well-sm no-shadow" style="margin-top: 10px;">
                        Contract closed
                    </p>
                    @endif @endif @if (auth()->check()) @if($contract->signed_contract_file !='' &&
                    (auth()->user()->isAdmin()
                    || auth()->user()->isLegal()))

                    <p class="col-md-6 text-blue well well-sm no-shadow" style="margin-top: 10px;">
                        Contract closed
                    </p> @endif @endif @if (auth()->check()) @if($contract->contract_status =='Approved' &&
                    (auth()->user()->isUser()
                    || $contract->created_by == Auth::user()->id))
                    <a href="#modal_upload_signed_contract" data-backdrop="static" data-keyboard="false"
                        data-toggle="modal" data-target="#modal_upload_signed_contract" class="btn btn-success"><i
                            class="fa fa-check"></i> Upload Signed Contract</a> @endif
                    ...
                    <a href="/{{$last_draft_contract_section->crf_form}}" class="btn btn-primary pull-right"
                        style="margin-right: 10px;" target="_blank"><i class="fa fa-fw fa-download"></i> CRF
                        Document</a> @endif

                    <a href="/{{$last_draft_contract_section->draft_file}}" class="btn btn-primary pull-right"
                        style="margin-right: 10px;" target="_blank"><i
                            class="fa fa-fw fa-download"></i>@if($contract->contract_status =='Closed') Signed Contract
                        Document @else Latest Contract Document @endif</a>{!!
                    Form::close() !!}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary btn-flat"><i class="fa fa-check"></i> Publish
                        Contract</button>
                </div>
                {!! Form::close() !!}
            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group" required=''>
                                ...
                                <label><input type="radio" name="contract_type" value="2" class="flat-red">&nbsp;&nbsp;
                                    Non Standard</label>
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">Upload Signed Contract(Signed by contracting party and initiating
                        department)</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i
                            class="fa fa-times"></i> Cancel</button>
                    <button type="submit" class="btn btn-primary btn-flat"><i class="fa fa-check"></i> Archive
                        Contract</button>
                    <script type="text/javascript">
                        $(".select2").select2();
                    </script>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i
                            class="fa fa-times"></i> Cancel</button>
                    <button type="submit" class="btn btn-primary btn-flat"><i class="fa fa-check"></i> Ammend
                        Contract</button>
                </div>
                {!! Form::close() !!}
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i
                            class="fa fa-times"></i> Cancel</button>
                    <button type="submit" class="btn btn-danger btn-flat"><i class="fa fa-check"></i> Terminate
                        Contract</button>
                </div>
                {!! Form::close() !!}
            </div>
                    ...
                    <table id="example1" class="table no-margin">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="width:120px"> <a href="/{{$contracts->draft_file}}" target="_blank"><i
                                            class="fa fa-fw fa-download"></i> Download</a></td>
                                @if($contracts->crf_form =='')
                                <td style="width:120px"> <a href="#"> No CRF Document</a></td>
                                @else
                                <td style="width:120px"> <a href="/{{$contracts->crf_form}}" target="_blank"><i
                                            class="fa fa-fw fa-download"></i> Download</a></td>
                                @endif
                                <td>
                                    <center><span class="pull-right-container">
                                            ...
                                            <small
                                                class="badge bg-purple">{{$contracts->contract_drafts_status}}</small></span>
                                        @elseif($contracts->contract_drafts_status== 'Pending')
                                        <small
                                            class="badge bg-yellow">{{$contracts->contract_drafts_status}}</small></span>
                                        @elseif($contracts->contract_drafts_status== 'Approved')
                                        <small
                                            class="badge bg-green">{{ $contracts->contract_drafts_status}}</small></span>
                                        @elseif($contracts->contract_drafts_status== 'Closed')
                                        <small
                                            class="badge bg-aqua">{{ $contracts->contract_drafts_status}}</small></span>
                                        @elseif ($contracts->contract_drafts_status== 'Amended')
                                        <small
                                            class="badge bg-blue">{{$contracts->contract_drafts_status}}</small></span>
